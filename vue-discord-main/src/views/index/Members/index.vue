  ...
  <div class="p-4 custom-scrollbar overflow-y-scroll">
    ...
    <div v-for="type in ['online', 'offline']">
      ...
      >
        ...
        <div
          v-else
          class="w-9 h-9 rounded-full bg-cover bg-center flex-shrink-0"
          :style="{ 'background-image': userPhotoURL(user) }"
        />

        <div
          class="flex flex-col justify-between gap-1 max-w-[100%] flex-shrink-[99] truncate"
        >
          <span
          ...
          <span
            ...
            class="text-xs w-fit text-white_500 rounded-xl px-1.5"
          >
            {{ user.role.name }}
          </span>
        </div>
      ...
      return [
        {
          ...
          event: "kickUser",
          condition:
            this.permissions?.kickUser &&
            this.selectedServer?.creator !== this.contextMenuItem?._id,
        },
        {
