<template>
  <DialogWrapper :title="$tc('server.leave', 2, {name: server?.name})" v-model="show">
    ...
    <div class="flex gap-4 justify-end items-center">
      ...
      <button @click="leave" class="bg-red-500 text-white_500 rounded px-3 py-2.5">{{ $t('server.leave') }}</button>
    </div>
  </DialogWrapper>
</template>
