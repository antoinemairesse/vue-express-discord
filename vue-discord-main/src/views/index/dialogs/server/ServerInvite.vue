  ...
  <DialogWrapper :title="'Invite friends to Server de Antoine'" v-model="show">
    <div v-if="!edit">
      <div class="flex flex-col mb-5">
        ...
        <div class="flex rounded-md bg-white_500">
          <input
            type="text"
            class="rounded-l-md py-2 px-2 w-full outline-0"
            name="name"
            v-model="link"
            readonly
          />
        ...
        <span class="text-label text-xs mt-2">
          <span v-if="expirationString !== 'Never'"
            >{{
              $tc("invite.expires_in", 2, { expiration: expirationString })
            }}
            &nbsp;</span
          >
          <span v-else>{{ $t("invite.never_expires") }} &nbsp;</span>
          <span class="cursor-pointer text-link" @click="edit = true">{{
            $t("invite.edit")
          }}</span>
        </span>
      </div>
    ...
    <div v-else>
      ...
      <div class="flex gap-4 justify-end items-center">
        ...
        <button
          class="bg-purple text-white_500 rounded px-3 py-2.5"
