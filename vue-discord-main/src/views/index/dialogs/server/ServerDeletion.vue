<template>
  <DialogWrapper :title="$tc('server.delete', 2, {name: server?.name})" v-model="show">
    ...
    <Form @submit="_delete" :validation-schema="schema">
      ...
      <div class="flex gap-4 justify-end items-center">
        ...
        <button type="submit" class="bg-red-500 text-white_500 rounded px-3 py-2.5">{{ $t('server.delete') }}
        </button>
      </div>
    </Form>
  </DialogWrapper>
  ...
  methods: {
    toggle(server) {
      ...
      this.schema = yup.object({
        name: yup.string().required().oneOf([server?.name], this.$t('server.name_confirm_error')),
      });
      this.show = !this.show;
    },
    _delete() {
