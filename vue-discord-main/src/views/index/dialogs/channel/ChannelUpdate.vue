<template>
  <DialogWrapper :title="$tc('channel.edit', 2, {name: channel?.name})" v-model="show">
    <Form @submit="update" class="flex flex-col" :validation-schema="schema">
      ...
      <div class="flex gap-4 justify-end items-center">
        ...
        <button class="bg-purple text-white_500 rounded px-3 py-2.5">{{ $t('channel.edit') }}</button>
      </div>
    </Form>
  </DialogWrapper>
</template>
...
export default {
  ...
  methods: {
    ...
    update(data) {
      this.$api.channel.update(this.channel?._id, data)
