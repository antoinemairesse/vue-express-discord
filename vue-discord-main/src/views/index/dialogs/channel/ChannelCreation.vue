  ...
  <DialogWrapper :title="$t('form.buttons.create_channel')" v-model="show">
    <Form @submit="create" class="flex flex-col" :validation-schema="schema">
      ...
      <div class="flex gap-4 justify-end items-center">
        <button
          type="button"
          class="text-white_500 hover:underline"
          @click="show = false"
        >
          {{ $t("cancel") }}
        </button>
        <button
          class="bg-purple text-white_500 rounded px-3 py-2.5 disabled:cursor-not-allowed disabled:opacity-50"
...
export default {
  ...
  methods: {
    ...
    create(data) {
      ...
      this.createChannel(data).finally(() => (this.show = false));
    },
  },
};
</script>
